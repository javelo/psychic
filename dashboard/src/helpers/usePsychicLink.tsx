...
const PSYCHIC_URL = import.meta.env.VITE_PSYCHIC_LINK_OAUTH_URL;
const PSYCHIC_API_URL = import.meta.env.VITE_PSYCHIC_API_URL;
...
export function usePsychicLink(public_key: string, public_api_url: string, onSuccessCallback: Function) {
  ...
  async function getCustomAuthUrl(public_key: string) {
    try {
      ...
      const response = await fetch(`${public_api_url ?? PSYCHIC_API_URL}/get-link-settings`, {
        method: 'GET',
        headers: {
